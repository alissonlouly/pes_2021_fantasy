...
if not df_time.empty:
    ...
    if custo_total > 170:
        st.error(f"🚨 Custo total ultrapassou o limite de 170 moedas! ({custo_total:.1f})")
    else:
        st.markdown(f"""
            <div class="big-number-card">
                💵 Custo total do time: {custo_total:.1f} moedas
            </div>
        """, unsafe_allow_html=True)
...
# =====================
if len(titulares) == 11 and len(reservas) <= 12 and df_time["preco"].sum() <= 170:
    ...
    with col_field:
        if formacao == "4-3-3":
            posicoes_formacao = [
                ("GK", 90, 50), ("LB", 70, 15), ("CB1", 70, 40), ("CB2", 70, 60), ("RB", 70, 85),
            ...
            posicoes_formacao = [
                ("GK", 90, 50), ("LB", 70, 15), ("CB1", 70, 40), ("CB2", 70, 60), ("RB", 70, 85),
                ("LM", 35, 20), ("CM1", 50, 40), ("CM2", 50, 60), ("RM", 35, 80),
                ("ST1", 15, 35), ("ST2", 15, 65)
            ]
        ...
        for i, (pos, top, left) in enumerate(posicoes_formacao):
            if i < len(titulares):
                ...
                jogador_text = ""
            html_campo += f'<div class="jogador" style="top:{top}%; left:{left}%; transform:translate(-50%, -50%);">{jogador_text}</div>'
        html_campo += "</div>"
